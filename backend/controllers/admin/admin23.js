...
exports.updateAdminCollection = async (db, req, res) => {
    ...
    await db.query(adminQueries.updateAdminCollection, [collection, 1], function (error, data) {
        if (error) {
            ...
            return res.status(200).send({
                success: true,
                msg: "Promo Coin Charges Updated Successfully",
                data
            });
        } else {
            return res.status(400).send({
...
exports.addBlog = async (db, req, res) => {
    ...
    await db.query(adminQueries.addBlog, [collection, 1], function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error : Server is not responding please try again later",
...
exports.listItem = async (db, req, res) => {
    ...
    var qry = ` Select i.id,ie.id as item_edition_id, case when length(i.name)>=30 then concat(left(i.name,30),'...')  else i.name end as name,i.name as item_fullname,i.description,i.image,i.file_type,i.owner,i.item_category_id,i.token_id,coalesce(ie.price,'') as price,coalesce(i.start_date,i.datetime) as start_date,i.end_date,ie.edition_text,ie.edition_no,ie.is_sold,ie.expiry_date,concat('${config.mailUrl}backend/infinity8_backend/uploads/',i.local_image) as local_image from item_edition as ie left join item as i on i.id=ie.item_id where  ie.is_sold=0 and ie.id in (select min(id) from item_edition where is_sold=0 group by item_id,owner_id)  and (ie.expiry_date > now() or ie.expiry_date is null) and i.is_active=1 and ie.owner_id<>1 and ie.owner_id in (select id from users where is_featured=0)`;
